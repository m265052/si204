    //Will Pitkin       m265052
    //Proj01 Pt4        11Feb24
    //this part will implement the green screen with a row shifted image
    ...
    int main()
    {
        ...
        //determine if the row shift is allowed
        if(heightfg + rs > heightbg)
        {
        ...
        for(int i=0; i < heightbg; i++)
        {
            ...
            for(int j=0; j < widthbg; j++)
            {
                ...
                //happen if the row is within the rowshift
                if(i >= rs && i <= rs + heightfg && j < widthfg)
                {
                    fin >> rfg >> gfg >> bfg;
